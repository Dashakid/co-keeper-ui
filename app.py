...
def train_model_api(uploaded_file):
    try:
        ...
        endpoint = f"{BACKEND_URL}/train_xero" if st.session_state.active_pipeline == 'xero' else f"{BACKEND_URL}/train_qb"
        ...
        if response.status_code == 200:
            ...
            return None, f"API Error ({response.status_code}): {response.json().get('detail', 'Unknown error')}"
    except requests.exceptions.ConnectionError:
        return None, f"Cannot connect to backend at {BACKEND_URL}."
    except requests.exceptions.Timeout:
        return None, "Training request timed out."
    except Exception as e:
...
def predict_model_api(prediction_data):
    try:
        ...
        endpoint = f"{BACKEND_URL}/predict_xero" if st.session_state.active_pipeline == 'xero' else f"{BACKEND_URL}/predict_qb"
        response = requests.post(endpoint, files=files, timeout=300)
        ...
        return None, "Prediction request timed out."
    except Exception as e:
        return None, f"Error: {str(e)}"
...
if page == "upload":
    try:
        ...
        backend_ok = health_response.status_code == 200
    except Exception:
        backend_ok = False
            ...
            if st.session_state.active_pipeline == 'xero':
                ...
                if is_valid:
                    ...
                    with st.expander("Preview prediction data"):
                        st.dataframe(df_pred.head(8), use_container_width=True)
                else:
                    st.error(message)
...
# ============================================================================
    ...
    if st.session_state.results is None:
        ...
        with col1:
            if st.button(f"🟢 GREEN  ·  {green_n:,} rows  ({green_n/total*100:.0f}%)", use_container_width=True, key="btn_green"):
                st.session_state.selected_tier = 'GREEN'

        with col2:
            if st.button(f"🟡 YELLOW  ·  {yellow_n:,} rows  ({yellow_n/total*100:.0f}%)", use_container_width=True, key="btn_yellow"):
                st.session_state.selected_tier = 'YELLOW'

        with col3:
            if st.button(f"🔴 RED  ·  {red_n:,} rows  ({red_n/total*100:.0f}%)", use_container_width=True, key="btn_red"):
